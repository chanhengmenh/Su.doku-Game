...
class Table:
    ...
    def _draw_buttons(self):
        ...
        # Adding "Restart" button details
        restart_button_color = pygame.Color(153, 101, 21) # Dark gold color
        self.restart_button = pygame.Rect(0, (height + cell_size[1]), (cell_size[0] * 3), (cell_size[1]))
        pygame.draw.rect(self.screen, restart_button_color, self.restart_button)
        restart_msg = self.font.render("Restart", True, self.font_color)
        self.screen.blit(restart_msg, (self.restart_button.x + (cell_size[0] // 2), self.restart_button.y + (cell_size[1] // 4)))
       
        exit_button_color = pygame.Color(153, 101, 21) # Dark gold color
        self.exit_button = pygame.Rect((cell_size[0] * 6), (height + cell_size[1]), (cell_size[0] * 3), (cell_size[1]))
        pygame.draw.rect(self.screen, exit_button_color, self.exit_button)
        exit_msg = self.font.render("Exit", True, self.font_color)
        self.screen.blit(exit_msg, (self.exit_button.x + (cell_size[0] // 3), self.exit_button.y + (cell_size[1] // 4)))
    ...
    def handle_mouse_click(self, pos):
        ...
        if self.exit_button.collidepoint(pos):
            print("Exitted Sudoku!")
            pygame.quit()
            sys.exit()
               
        # Restart game logic
        if self.restart_button.collidepoint(pos):
            print("Restarting Sudoku!")
            # Logic to restart the game
            self._generate_game()
            self.clock.start_timer()
            self.clock.elapsed_time = 0
            self.clock.countdown_time = 10 * 60
    ...
    def handle_keyboard_input(self, key):
        ...
        if self.clicked_cell and self.clicked_cell.value == 0:
            ...
            if key < pygame.K_0 or key > pygame.K_9:
                self.lives += 1
                print(f"Non-numeric key pressed, so live:{self.lives}")
    ...
    def update(self):
        ...
        self._draw_grid()
        self._draw_buttons()
        if self._puzzle_solved() or self.lives == 0 or self.clock.elapsed_time >= self.clock.countdown_time:
