...
class Sudoku:
    ...
    #initialize the class with size of Sudoku Board and number of empty cells to remove
    def __init__(self, N, E):
        self.N = N      # N is the size of the Sudoku board
        self.E = E      # E is the number of empty cells to remove
        
        # compute square root of N
        self.SRN = int(math.sqrt(N))

        # Initialize the Sudoku board as a list of lists, where each sublist represents a row
        self.table = [[0 for x in range(N)] for y in range(N)]
        # Initialize a second list of lists that will contain the Sudoku board with the empty cells removed
    ...
    # Remove the specified number of empty cells from the Sudoku board
    def remove_digits(self):
        count = self.E      # Count is the number of empty cells to remove
